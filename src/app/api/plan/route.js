...
export async function POST(request) {
  try {
    ...
    if (!isSecondPass) {
      ...
      // Extract tone/mood if mentioned in the user's answers
      const toneMatch = videoTopic.match(/tone[:\s]+([^.\n]+)/i) ||
                        videoTopic.match(/mood[:\s]+([^.\n]+)/i) ||
                        videoTopic.match(/style[:\s]+([^.\n]+)/i);
      const userTone = toneMatch ? toneMatch[1].trim() : null;
      const toneInstruction = userTone
        ? `CREATOR-SPECIFIED TONE: "${userTone}" — this is NON-NEGOTIABLE. The entire thumbnail concept, headline text, overlays, and subject expression MUST reflect this tone exactly. For example, if humorous: use funny/meme-style text, exaggerated expressions, playful elements. If serious: bold urgent text, intense expression.`
        : "Match the natural tone of the topic.";
      ...
      systemPrompt = [
        ...
        toneInstruction,
        "",
        "Use your own judgment to decide which overlay elements genuinely fit the content AND the creator's specified tone.",
        ...
        "- If humorous/funny: use meme-worthy text, wordplay, exaggeration",
      ...
      userPrompt = [
        ...
        userTone ? `TONE (creator specified): ${userTone}` : "",
        "",
        ...
        `  "tone": "${userTone || "neutral"}",`,
        '  "subjectPose": "A descriptive pose that MATCHES THE TONE and emotion of the topic (e.g. for humorous: wide eyes with hands on cheeks in exaggerated shock, for serious: stern forward gaze, for celebratory: fist pump). Do NOT default to pointing upward unless it genuinely fits.",',
        '  "overlayConfig": {',
        ...
        '    "headline1": "FIRST LINE — uppercase max 3 words (white text) — MUST REFLECT TONE",',
        '    "headline2": "SECOND LINE — uppercase max 3 words (accent color) or null — MUST REFLECT TONE",',
...
function buildCompleteThumbnailPrompt(plan, { brandColor, highlightColor, hasSubjectPhoto, subjectCount, poseMode, videoTopic }) {
  ...
  // Extract tone from plan (set by DeepSeek) or from the enriched topic string
  const tone = plan.tone ||
    (videoTopic && (() => {
      const m = videoTopic.match(/tone[:\s]+([^.\n]+)/i) || videoTopic.match(/mood[:\s]+([^.\n]+)/i);
      return m ? m[1].trim().toLowerCase() : null;
    })()) || "neutral";

  const isHumorous = /humour|humor|funny|comedy|meme|joke|comic|witty|playful|fun/i.test(tone);
  const isUrgent   = /urgent|serious|critical|important|breaking|emergency/i.test(tone);
  const isInspiring = /inspir|motivat|uplifting|positive|celebrat/i.test(tone);

  // Tone-specific style modifier
  const toneStyleNote = isHumorous
    ? "TONE: Humorous and meme-worthy. The subject should have an exaggerated, over-the-top comedic expression (wide eyes, dropped jaw, hands-on-cheeks panic face, or mischievous grin). Text should feel playful and funny — like a viral meme thumbnail. The overall mood is light-hearted and funny, not serious."
    : isUrgent
    ? "TONE: Urgent and serious. Bold, no-nonsense energy. Intense subject expression, high-contrast colors, urgent layout."
    : isInspiring
    ? "TONE: Inspiring and motivational. Warm lighting, triumphant subject pose, uplifting colors."
    : "TONE: Balanced and professional. Clean, impactful design.";
